...
export default class BabyLifeListView extends React.Component<any, any> {
    ...
    _getCommonMilkDose(dataList: any) {
        if (dataList) {
            ...
            if (oldDataList.length > 0) {
                ...
                mainData.oldData.oldMilkData = JSON.parse(
                    JSON.stringify(oldDataList[0]),
                );
                // 拿到最新的3个
    ...
    _getCommonPoopDose(dataList: any) {
        if (dataList) {
            ...
            if (oldDataList.length > 0) {
                ...
                mainData.oldData.oldPoopData = JSON.parse(
                    JSON.stringify(oldDataList[0]),
                );
                // 拿到最新的3个
    ...
    _getCommonPeeDose(dataList: any) {
        if (dataList) {
            ...
            if (oldDataList.length > 0) {
                mainData.oldData.oldPeeData = JSON.parse(
                    JSON.stringify(oldDataList[0]),
                );
                // 拿到最新的3个
                let subList = oldDataList.slice(0, 3);
            ...
            <Modal
                ...
                <View style={styles.addModalContainer}>
                    <View style={styles.addContentContainer}>
                        ...
                        {contentView ? (
                            <View style={{flex: 1, padding: 12}}>{contentView}</View>
                        ) : null}
                        {footerView ? <View style={styles.line}/> : null}
                        {footerView ? (
    ...
    _renderMilkContent(typeData) {
        ...
        let tagView = renderTagList(
            ...
            tag => {
                ...
                // 单选
                this.cloneType.selectedTags.splice(
                    0,
                    this.cloneType.selectedTags.length,
                );
    ...
    _confirmOtherNewLife(callback) {
        if (this.isTypeEdit) {
            ...
            if (this._checkAddTypeData(this.cloneType)) {
                ...
                let dataList = this._insertItemByResortTime(
                    this.state.dataList,
                    this.cloneType,
                );
                this.setState({
    ...
    _renderSvgCharts() {
        return (
            <PagerView
                style={{width: screenW, height: screenW * 0.6, flex: 1}}
                initialPage={0}>
